...
export class MediaMTXWebRTCReader {
  ...
  static supportsNonAdvertisedCodec(codec: any, fmtp: any) {
  ...
  static unquoteCredential(v: string) {
    return JSON.parse(`"${v}"`);
  }

  static linkToIceServers(links: string | null) {
    return links !== null
  ...
  static enableStereoPcmau(payloadTypes: any, section: string) {
  ...
  static enableMultichannelOpus(payloadTypes: any, section: string) {
    const lines = section.split("\r\n");
  ...
  static enableL16(payloadTypes: any, section: string) {
    const lines = section.split("\r\n");
  ...
  static generateSdpFragment(
    od: { iceUfrag: any; icePwd: any; medias: any },
    candidates: any
  ) {
    const candidatesByMedia: { [key: number]: any[] } = {};
  ...
  async setupPeerConnection(iceServers: any) {
    if (this.state !== "running") {
      throw new Error("closed");
    }
  ...
  onLocalCandidate(evt: { candidate: RTCIceCandidate | null }) {
    if (this.state !== "running") {
      return;
    }
  ...
  sendLocalCandidates(candidates: any) {
    if (!this.sessionUrl || !this.offerData) {
      console.error("Session URL or offer data is not set");
      return;
    }
    fetch(this.sessionUrl, {
      method: "PATCH",
